...
def predict_hurricane_path(start_lat, start_long, start_wind, start_pressure, steps=24):
    initial_sequence = np.array([[start_lat, start_long, start_wind, start_pressure]] * seq_length)
    ...
    for _ in range(steps):
        next_step = model.predict(current_sequence.reshape(1, seq_length, len(features)))
    ...
    features_predictions = scaler.inverse_transform(predictions)
